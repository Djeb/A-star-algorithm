...
class Astar{
	...
	ifstream in;
public:
	float cost(Orientation a, Orientation b){
		return std::sqrt(std::pow((b.lon - a.lon), 2.0) + std::pow((b.lat - a.lat), 2.0));
	}
	void start(unsigned int start, unsigned int goal){
		...
		while(!frontier.empty()){
			...
			for (int i = 0; i < web[current].lsize(); ++i){
				...
				if (new_cost < cost_far[next]){
					...
					int priority = new_cost + ManhattanHeuristic(web[current].get_link(i).get_orient(), web[goal].get_orient());
	...
	bool read_raw(char* filename){
                ...
                if(!in)
               	...
               	while(!in.eof()){
					...
					float l = cost(Orientation(lon1, lat1), Orientation(lon2, lat2));
					if(web.size() <= (a > b? a:b)){
